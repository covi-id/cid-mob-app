...
import {
   View, TouchableOpacity, TouchableNativeFeedback, Text, Dimensions, Animated, ActivityIndicator, Platform,
} from 'react-native';
...
export default function StyledButton({
   onPress, loading, loadingWidth, disabled, alternative, dark,
   auth, flat, iconLeft, iconRight, title, titleColor, ...props
}) {
   const {
      colors, sizes, fonts,
   } = useTheme();
   ...
   const styles = {
      ...
      contentContainer: {
         ...
         backgroundColor: disabled && !alternative ? colors.primaryAccent : auth ? '#27066B' : alternative ? colors.secondary : dark ? colors.background : colors.primary,
      ...
      text: {
         color: titleColor || (dark || !alternative || (!alternative && disabled) ? colors.background : colors.primary),
   ...
   const RootElement = (props) => (Platform.OS === 'android' ? (
      ...
      <TouchableOpacity onPress={onPress} style={props.style} disabled={props.disabled}>
         {props.children}
      </TouchableOpacity>
   ));
...
function getSvg(icon, color) {
   switch (icon) {
      case 'edit': return <EditSvg color={color} />;
      case 'arrow': return <ArrowSvg color={color} />;
      default: return null;
   }
}
